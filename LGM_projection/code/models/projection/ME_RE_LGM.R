...
resolution = 2.5 # worldclim resolution
### RUNNING THE RANDOM FOREST MODELS

## Actions performed in this code:
# 1 - read in data and get new LGM bioclim factors, then transform with PCA
...
c.americana <- read.csv("data/RearEdge_pa2m30s_LGM.csv") %>% na.omit() # 2 rows of NA's (coordinates are in ocean by NYC which escaped mask filter (resolution issues?)
...
if (exists("bio.crop") == FALSE) {
  ...
  bio.crop <- terra::extract(rast(lgm), vect(world.crop), cells=T, xy=T)
...
jpeg("plots/maxent/forward_proj/me_RearEdge_LGM.jpeg", width=10, height=10, units="in", res=500)
ggplot()+
...
write.csv(pred.grid, "data/models/maxent/forward_proj/me_LGM.csv", row.names = F)

# save raster
pred.raster <- rasterFromXYZ(pred.grid[,c("longitude", "latitude", "pred.cell")],
                             crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
plot(pred.raster) # check that it worked
writeRaster(pred.raster, 'data/models/maxent/forward_proj/me_LGM.tif', overwrite=T)


# 4 - predict contemporary using contemporary model framework
...
# crop data to area of interest
bio.lgm.crop <- terra::extract(bio.world, vect(world.crop), cells=T, xy=T)
...
# iv. generate predictions from all models for range extent +/- 3 lat/long
pred.lgm.cell <- predict(m_me, bio.lgm.pcN)
...
jpeg("plots/maxent/forward_proj/me_RearEdge_contemporary.jpeg", width=10, height=10, units="in", res=500)
ggplot()+
  geom_sf(data=world.sf, fill=NA)+
  geom_sf(data=states, fill=NA)+ #adds state boundaries
...
# save files for export
write.csv(pred.lgm.grid, "data/models/maxent/forward_proj/me_contemporary.csv", row.names = F)

# save raster
pred.lgm.raster <- rasterFromXYZ(pred.lgm.grid[,c("longitude", "latitude", "pred.lgm.cell")],
                             crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
plot(pred.lgm.raster) # check that it worked
writeRaster(pred.lgm.raster, 'data/models/maxent/forward_proj/me_contemporary.tif', overwrite=T)
