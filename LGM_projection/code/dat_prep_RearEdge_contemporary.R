...
cluster.size = 1 # number of neighbors within given distance. change to 0 if not desired
...
# 7. saving data 
if (resolution == 0.5) {
  write.csv(c.americana, "data/RearEdge_pa30s.csv", row.names = F)
}

if (resolution == 2.5) {
  write.csv(c.americana, "data/RearEdge_pa2m30s.csv", row.names = F)
}

# save plot
jpeg("plots/data_prep2m30s.jpeg", res=5e2, units="in", height=5, width=6)
ggplot(data=world.sf)+
  geom_sf(data=states, fill=NA, alpha=0.5, color="black")+ #adds state boundaries
  geom_sf(data=lakes, fill="white")+ #adds the great lakes
  geom_sf(data=r.sf, fill=NA, color="gray", alpha=0.3)+ # full mask
