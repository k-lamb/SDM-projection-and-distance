...
jpeg("plots/rf/forward_proj/m_rf_100avg_RearEdge_LGM.jpeg", width=10, height=10, units="in", res=500)
ggplot()+
...
write.csv(pred.grid, "data/models/rf/forward_proj/rf_100avg_LGM.csv", row.names = F)

# save raster
pred.raster <- rasterFromXYZ(pred.grid[,c("longitude", "latitude", "pred.cell")],
                             crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
plot(pred.raster) # check that it worked
writeRaster(pred.raster, 'data/models/rf/forward_proj/rf_100avg_LGM.tif', overwrite=T)
...
jpeg("plots/rf/forward_proj/m_rf_100avg_RearEdge_contemporary.jpeg", width=10, height=10, units="in", res=500)
ggplot()+
  geom_sf(data=world.sf, fill=NA)+
  geom_sf(data=states, fill=NA)+ #adds state boundaries
...
# save files for export
write.csv(pred.lgm.grid, "data/models/rf/forward_proj/rf_100avg_contemporary.csv", row.names = F)

# save raster
pred.lgm.raster <- rasterFromXYZ(pred.lgm.grid[,c("longitude", "latitude", "pred.lgm.cell")],
                             crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
plot(pred.lgm.raster) # check that it worked
writeRaster(pred.lgm.raster, 'data/models/rf/forward_proj/rf_100avg_contemporary.tif', overwrite=T)
