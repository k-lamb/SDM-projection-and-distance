...
pred.list <- lapply(saveme, mecofun::predictSDM, newdata=bio.pcN) # 14 seconds per model
...
jpeg("code/final/plots/rf/m_rf_10avg.jpeg", width=10, height=10, units="in", res=500)
ggplot()+
...
write.csv(pred.grid, "code/final/data/models/rf/rf_10avg.csv", row.names = F)

# save raster
pred.raster <- rasterFromXYZ(pred.grid[,c("longitude", "latitude", "pred.cell")],
                             crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
plot(pred.raster) # check that it worked
writeRaster(pred.raster, 'code/final/data/models/rf/rf_100avg.tif', overwrite=T)
