...
cluster.size = 1 # number of neighbors within given distance. 
nearest.neighbor.TF = F # set to F if no nearest.neighbor restriction is desired (most useful for full range scale)
buffer.dist.N = 30 # buffer distance for restricting presence points by phylogenetic lineage
buffer.dist.PA = 100 # buffer distance for restricting generation of pseudo-absence points
resolution = 2.5 # worldclim resolution. finer resolution takes longer (0.5, 2.5, etc.)
pa.rep = 5 # multiplier for creating pseudo-absence data
...
# 1 - Import sampling data
# 2 - generate a buffered concavity around the sampling data to obtain points from iNaturalist
# 3 - Import iNaturalist data and limit to concavity
# 4 - Spatial down-sampling of presence data
# 5 - Generate pseudo-absence data bound by some reasonable proximity to known points
# 6  - Download/import WorldClim 2.1 data
# 7 - Save data


# 1. import genetic sampling data with known phylogeographic assignment

# generating subsample using the full records list and cutoff of 35°N
# gen.coord <- read_excel("~/Dropbox/Galloway Lab Folder 2022/genomics/Info seq 2023/Samples detail.xlsx", sheet = 1) %>% 
#   as.data.frame() %>% 
#   filter(Clade == "Western") %>%
#   mutate_at(vars(lat, long), as.numeric) %>% 
#   mutate(lat = round(lat, 2)) %>% 
#   mutate(long = round(long, 2)) %>% 
#   dplyr::select(POP...1, lat, long)
# 
# names(gen.coord) <- c("pop", "latitude", "longitude")
# gen.sample <- gen.coord %>% 
#   filter(latitude <= 35)

# generating subsample using phylogeographic description
gen.sample <- read.csv("data/pops rear edge.txt", sep="\t")
names(gen.sample) <- c("pop", "longitude", "latitude", "clust")

# 2. generate a buffered concave polygon around the desired samples
gen.sf <- st_as_sf(gen.sample, coords=c("longitude", "latitude"), crs=4326)
gen.hull <- concaveman(gen.sf)
gen.buffer <- st_buffer(gen.hull, dist = buffer.dist.N*1e3)


## 3. Import and prep iNaturalist data
# i - import iNaturalist data
# ii - apply sampling buffer to data

# i. import
coordinates.pts <- read.csv("./data/iNaturalist_coords.csv", sep=',') # data from iNaturalist to May 2022

# ii. apply buffer around sampling to grab points of interest
coordinates.sf <- st_as_sf(coordinates.pts[,c("longitude", "latitude")], coords=c("longitude", "latitude"), crs=4326)
...
# 34°N keep everything below
if (lat.cutoff.TF == TRUE) {
  coord.subN <- coordinates.full %>% filter(latitude >= lat.cutoff)
  coord.subS <- coordinates.pts %>% filter(latitude < lat.cutoff) %>% dplyr::select(longitude, latitude)
  coordinates.full <- rbind(coord.subN, coord.subS) %>% as.data.frame()
}

## 4. Spatial down-sample of presence data
...
if (sanity.plot == T) {
  ggplot(data=world.sf)+
    ...
    geom_sf(data=gen.buffer, fill=NA)+
    ...
    geom_hline(yintercept = lat.cutoff, linetype=2)+
    geom_point(data=coordinates.pts, aes(x=longitude, y=latitude), size=0.5, color="skyblue")+ # all iNaturalist
    geom_point(data=coordinates, aes(x=longitude, y=latitude), size=0.5)+ # restricted set of iNaturalist
    geom_point(data=gen.sample, aes(x=longitude, y=latitude), size=1.5, color="red")+ # genetic sampling (have sequences)
...
## 5. Generate pseudo-absence data
...
buffer.hull <- st_buffer(concave.hull, dist = buffer.dist.PA*1e3) # takes ~2 minute to run
...
# increase number of points generated using for loop
...
pa.grid <- round(latlong2grid(pa.df)) #gets km grid and rounds to nearest whole km
...
## 6. download and prep bioclim data from WorldClim 2.1
...
cam.bio <- terra::extract(lgm, cam.sf) 
...
# 7. saving data 
if (resolution == 0.5) {
  write.csv(c.americana, "data/RearEdge_pa30s_LGM.csv", row.names = F)
}

if (resolution == 2.5) {
  write.csv(c.americana, "data/RearEdge_pa2m30s_LGM.csv", row.names = F)
}

# save plot
jpeg("plots/data_prep2m30_LGMs.jpeg", res=5e2, units="in", height=5, width=6)
ggplot(data=world.sf)+
  geom_sf(data=states, fill=NA, alpha=0.5, color="black")+ #adds state boundaries
  geom_sf(data=lakes, fill="white")+ #adds the great lakes
  geom_sf(data=r.sf, fill=NA, color="gray", alpha=0.3)+ # full mask
  geom_sf(data=concave.hull, fill=NA, color="red4")+ # concave hull
  geom_sf(data=buffer.hull, fill=NA, color="skyblue2", linewidth=0.5)+ # buffer hull (used in masking)
  geom_point(data=c.americana %>% filter(presence == 0), aes(x=longitude,y=latitude), 
             color="coral1", size=0.75)+
  geom_point(data=c.americana %>% filter(presence == 1), aes(x=longitude, y=latitude), 
             color="forestgreen", size=0.75)+
  coord_sf(xlim=c(min(coordinates$longitude)-3, max(coordinates$longitude)+3), 
           ylim=c(min(coordinates$latitude)-3, max(coordinates$latitude)+3), expand=FALSE)+
  xlab("Latitude")+
  ylab("Longitude")+
