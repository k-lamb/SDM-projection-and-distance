    ...
    tryCatch(
     {
        if (length(unique(sub[,"presence"] )) == 2) { # want to avoid any windows where there are only absences or presences
          m_rf <- randomForest(x=sub[,paste0("PC", 1:pc.num)], y=as.factor(sub[,"presence"]), ntree=500, importance=T)
          pred <- predictSDM(m_rf, bio.sub2[,paste0("PC", 1:pc.num)])
...
# arrange and group data so only highest prediction is retained per sliding window
pred.grid <- pred.df %>% 
...
jpeg("code/grid_search/plots/rf/rf_cellmax_baseline.jpeg", width=10, height=10, units="in", res=500)
ggplot()+
...
# write.csv(pred.df, "code/grid_search/data/models/rf/rf_all_windows.csv", row.names = F)
write.csv(pred.grid, "code/grid_search/data/models/rf/rf_cellmax_windows.csv", row.names = F)

# save raster
pred.raster <- rasterFromXYZ(pred.grid[,c("longitude", "latitude", "pred")],
                             crs = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
plot(pred.raster) # check that it worked
writeRaster(pred.raster, 'code/grid_search/data/models/rf/rf_cellmax.tif', overwrite=T)
...
write_stars(smooth.raster, 'code/grid_search/data/models/rf/rf_cellmax_smooth.tif')

smooth.raster <- raster("code/grid_search/data/models/rf/rf_cellmax_smooth.tif")
...
jpeg("code/grid_search/plots/rf/rf_cellmax_smooth.jpeg", width=10, height=10, units="in", res=500)
ggplot()+
  geom_sf(data=world.sf, fill=NA)+
  geom_sf(data=states, fill=NA)+ #adds state boundaries
